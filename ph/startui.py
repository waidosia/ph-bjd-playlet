...
from .upload.playet import upload_playet
...
class MainWindow(QMainWindow, Ui_Mainwindow):
    def __init__(self):
        ...
        self.sendPlayetButton.clicked.connect(lambda: self.upload("playet"))
...
class UploadHandler(QObject):
    ...
    def on_upload_finished(self, platform, success, message,link_or_error, path):
        ...
        if success:
            ...
            elif platform == "playet":
                self.parent.playetTorrentLink = link_or_error
                self.parent.playetTorrentPath = path
    ...
    def run(self):
        if self.platform == "tju":
            ...
            result = upload_dream(self.cookie_str,
                                  ...
                                  self.torrent_save_path,
                                  self.feed_checked)
        elif self.platform == "playet":
            result = upload_playet(self.cookie_str,
                                 self.torrent_path,
                                 self.mainTitle,
                                 self.secondTitle,
                                 self.introBrowser,
