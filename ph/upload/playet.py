...
def get_playet(cookies_str) -> (bool, str):
    global proxies
    headers = {
        'Host': 'playletpt.xyz',
        'Cookie': cookies_str,
        ...
        'Referer': 'https://playletpt.xyz/index.php',
        'Accept': '*/*',
    ...
    try:
        response = requests.get('https://playletpt.xyz/index.php', headers=headers, timeout=10, allow_redirects=False)
...
def upload_playet(cookies_str, torrent_file, main_title, compose, descr, media_info, proxy, torrent_path, feed) -> (
        bool, str):
    # 发布前，先请求一次主站，确定cookie是否是过期的
    get_success, get_str = get_playet(cookies_str)
    if not get_success:
    ...
    headers = {
        ...
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) '
                      'Chrome/112.0.5615.138 Safari/537.36',
        'Referer': 'https://playletpt.xyz/upload.php',
        'Accept': '*/*',
        'Connection': 'keep-alive',
        'Accept-Encoding': 'gzip, deflate',
        'Accept-Language': 'zh-CN,zh;q=0.9',
        'Origin': 'https://playletpt.xyz',
    ...
    data = {
        ...
        'codec_sel[4]': playet_video_codec_map.get(video_codec, '1'),
        # 音频编码 aac
        'audiocodec_sel[4]': '1',
        # 分辨率
        'standard_sel[4]': playet_video_codec_map.get(resolution, '0'),
        # 制作组
        'team_sel[4]': '7',
    ...
    try:
        response = requests.request("POST", 'https://playletpt.xyz/takeupload.php', headers=headers,
        ...
        if response.status_code == 302:
            ...
            time.sleep(1)
            get_success, torrent_link, torrent_save_path = get_playet_torrent(cookies_str, torrent_id, torrent_path)
...
def get_playet_torrent(cookies_str, torrent_id, torrent_path) -> (bool, str):
    global proxies
    headers = {
        'Host': 'playletpt.xyz',
        'Cookie': cookies_str,
    ...
    if torrent_path != '' and torrent_path is not None and torrent_path != 'None':
        try:
            response = requests.get(f'https://playletpt.xyz/download.php?id={torrent_id}', headers=headers, timeout=10,
                                    proxies=proxies)
            if response.status_code == 200:
                # 提取文件名
            ...
            return False, '下载种子失败', ""
    try:
        response = requests.get(f'https://playletpt.xyz/details.php?id={torrent_id}', headers=headers, timeout=10)
        if response.status_code == 200:
            # 获取种子下载链接
            html = response.text
